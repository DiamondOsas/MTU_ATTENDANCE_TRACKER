...
def extract_attendees(file_path, col_index, df=None):
    """
    Returns a list of students marked as absent (✓) in the specified column.
    ...
    try:
        ...
        absent_mask = clean_df[target_col].str.strip().isin(['✓'])
        attendees = clean_df[absent_mask]
        
        return attendees[['Surname', 'Firstname', 'Matric NO']].to_dict('records')

    except Exception as e:
        print(f"Error extracting attendees: {e}")
        return None

def save_attendees(attendees_data, file_path):
