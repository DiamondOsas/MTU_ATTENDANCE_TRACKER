...
from internal.absentees.abs_func import get_session_info, extract_absentees, save_absentees, load_attendance_file
...
class ChooseAbsenteeFileWindow(ChooseCSVWindow):
    ...
    def open_report(self, file_path):
        ...
        """
        PrintAbsenteesWindow(self.master, file_path)
...
class PrintAbsenteesWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master, file_path):
        ...
        self.current_absentees = [] # List of extracted data
        ...
        ctk.CTkButton(btn_frame, text="Show Absentees", command=self.show_absentees).pack(side="left", padx=10)
    ...
    def show_absentees(self):
        """Fetches and displays absentees for the selected range."""
        ...
        self.current_absentees = [] # Clear previous results
            ...
            for act in activities:
                ...
                if absentees:
                    ...
                    for person in absentees:
                        ...
                        self.current_absentees.append(person)
                        display_text += f"{person['Surname']} {person['Firstname']} ({person['Matric NO']})\n"
                else:
                    display_text += f"\n--- {d_str} : {act_name} (No Absentees) ---\n"
        ...
        if found_any:
            ...
            self.textbox_result.delete("0.0", "end")
            self.textbox_result.insert("0.0", "No absentees found for the selected period.")
    ...
    def export_data(self):
        ...
        if self.start_date != self.end_date:
            ...
            try:
                df = pd.DataFrame(self.current_absentees)
